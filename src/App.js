...
function App() {
  let customStyles;
  let [whiteTheme, setTheme] = useState(true);
  function toggleTheme() {
    ...
    if (!whiteTheme) {
      customStyles = {
        ...
        '--primary': '#f54748',
        '--secondary': '#fdc55e',
        '--gradient': 'linear-gradient(transparent, #f547474a)',
        '--theme': '#000000',
        '--filter': 'invert(0%)',
        '--background': '#fff',
      };
    ...
    let rootStyleString = Object.keys(customStyles).reduce((acc, key) => {
      return acc + `${key}: ${customStyles[key]};`;
    }, '');

    document.documentElement.style.cssText += rootStyleString;
